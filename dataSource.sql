...
insert into personsData (first_name, last_name, email, phone, gender) select first_name, last_name, email, phone, gender from dataSource01;
...
drop table if exists dataSource02;
create table dataSource02 as 
select 
case 
	when instr(lower(gender), 'f') = 0
		then 'male'
...
insert into personsData (first_name, last_name, email, phone, gender) select first_name, last_name, email, phone, gender from dataSource02;

select * from personsData;


/*
	взять актуальный срез
*/
...
inner join (
  ...
  and a.Created_at = b.maxCreated_at
...
drop view if exists personCom;
create view personCom as 
select 
first_name,
last_name,
created_at,
datetime(created_at, 'start of month') as communication_from,
datetime(created_at, 'start of month', '+3 month', '-1 second') as communication_to
from v_personsData;
